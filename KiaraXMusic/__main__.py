...
from KiaraXMusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("KiaraXMusic.plugins" + all_module)
    LOGGER("KiaraXMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Anony.start()
    try:
        await Anony.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("KiaraXMusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("KiaraXMusic").info("Stopping KiaraX Music Bot...")
